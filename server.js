...
app.post("/", (req, res) => {
  ...
  const { userId: user_id, userPassword: user_password } = req.body;

  // 3️⃣. (find 메서드를 사용하여) users의 정보와 사용자가 입력한 정보를 비교하여 일치하는 회원이 존재하는지 확인하는 로직을 작성하세요.
  const userInfo = users.find(user => 
    user.user_id === user_id && user.user_password === user_password);
...
app.delete("/", (req, res) => {
  ...
  req.session.destroy(err => {
    if(err){
      return res.status(500).send("세선 삭제 실패.");
    }
